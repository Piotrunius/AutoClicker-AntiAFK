...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _on_key_press(self, key):
        ...
        # MODIFIED: Prevent hotkeys from firing when typing in an input field
        ...
        try:
            ...
            return # It's a special key, not a character

        # MODIFIED: Check for Anti-AFK hotkey
        afk_hotkey = (self.afk_hotkey_edit.text() or "p").lower()
        if pressed_char == afk_hotkey:
            # Do not toggle AFK if autoclicker is currently clicking
            if not (self.worker and self.worker.isRunning()):
                self.on_toggle_afk_worker()
            return

        # MODIFIED: Check for AutoClicker hotkey
        activation_key = (self.activation_key_edit.text() or "r").lower()
        if pressed_char == activation_key:
            # Do not trigger autoclicker if anti-AFK is active
